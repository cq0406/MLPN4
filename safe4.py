...
if st.session_state.show_training:
    ...
    if is_cloud:
        # 云端环境：完全隐藏训练功能和特征重要性
        st.info("🔒 模型已内置。如需更新模型，请在本地训练后重新上传。")
        st.stop()
    else:
        # 本地环境：正常显示训练功能
        from sklearn.ensemble import RandomForestRegressor, RandomForestClassifier
        ...
        st.markdown("**模型文件将保存到桌面，请上传至 GitHub 供云端使用。**")
                ...
                if len(X) < 10:
                        ...
                        if can_train_cls:
                            if st.button("训练血药浓度达标预测模型（保存至桌面）", key="train_cls_stable"):
                                with st.spinner("训练中..."):
                                    ...
                                    st.success(f"分类模型训练完成！测试集准确率：{acc:.2%}")
                            ...
                            st.warning("❌ 分类目标只有一种类别，无法训练分类模型。请检查结论类型字段或增加不同疗效的病例。")
                    ...
                    if "pk_model" in st.session_state:
                        with st.expander("查看PK模型特征重要性"):
                            ...
                            imp_df = pd.DataFrame({"Feature": eng_features, "Importance": importances}).sort_values("Importance", ascending=False)
                            threshold = st.slider("有效特征重要性阈值", 0.0, float(imp_df["Importance"].max()), 0.02, 0.005, key="pk_threshold")
                    ...
                    if "outcome_model" in st.session_state:
                        with st.expander("查看血药浓度达标预测模型特征重要性"):
                            ...
                            threshold_cls = st.slider("有效特征重要性阈值", 0.0, float(imp_df_cls["Importance"].max()), 0.02, 0.005, key="cls_threshold")
