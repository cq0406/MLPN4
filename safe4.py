...
if st.session_state.show_training:
    ...
    from sklearn.ensemble import RandomForestRegressor, RandomForestClassifier
    from sklearn.metrics import mean_absolute_error, accuracy_score, mean_squared_error, r2_score, classification_report, ConfusionMatrixDisplay
    import joblib
    import os

    # 判断是否在 Streamlit Cloud 环境
    ...
    if is_cloud and (os.path.exists("pk_model.pkl") or os.path.exists("outcome_model.pkl")):
        ...
        # 显示已有模型的性能指标
        if os.path.exists("pk_model.pkl") and os.path.exists("pk_features.pkl"):
            try:
                model_reg = joblib.load("pk_model.pkl")
                features = joblib.load("pk_features.pkl")
                st.session_state.pk_model = model_reg
                st.session_state.pk_features = features
            except:
                pass
        if os.path.exists("outcome_model.pkl") and os.path.exists("outcome_encoder.pkl"):
            try:
                model_cls = joblib.load("outcome_model.pkl")
        ...
        if info:
            ...
            with col1:
                if "pk_mae" in info:
                    st.metric("PK模型测试集 MAE", f"{info['pk_mae']} mg/L")
                    st.caption(f"训练样本数：{info.get('pk_samples', '--')}")
            with col2:
                if "cls_acc" in info:
                    st.metric("疗效分类模型准确率", f"{info['cls_acc']:.1%}")
                    st.caption(f"训练样本数：{info.get('cls_samples', '--')}")
        else:
            st.info("暂无模型性能信息。可在本地训练时生成 model_info.json 并上传。")
    ...
    if len(df) < 10:
        ...
        if "给药前0.5h浓度" not in df.columns or "结论类型" not in df.columns:
            ...
            st.write("**原始结论类型分布：**", df["结论类型"].value_counts().to_dict())
                        ...
                        with st.spinner("训练中..."):
                            ...
                            if os.path.exists(info_path):
                                try:
                                    ...
                                    pass
                            info["pk_mae"] = round(mae, 2)
                            info["pk_samples"] = len(X_train)
                            info["pk_rmse"] = round(rmse, 2)
                            info["pk_r2"] = round(r2, 3)
                            with open(info_path, 'w', encoding='utf-8') as f:
                                json.dump(info, f, ensure_ascii=False, indent=2)

                            st.session_state.pk_model = model_reg
                            st.session_state.pk_features = all_feature_cols
                            st.success("模型已保存至桌面，model_info.json 已更新。")

                with col_tr2:
                    if can_train_cls:
                        if st.button("训练疗效分类模型（保存至桌面）", key="train_cls_stable"):
                            with st.spinner("训练中..."):
                                ...
                                st.success(f"疗效分类模型训练完成！测试集准确率：{acc:.2%}")
