...
if st.button("✅ 生成个体化给药方案"):
    ...
    try:
        ...
        # 尝试使用 ML 预测 CL/Vd 作为更优的先验
        prior_cl, prior_vd = cl_ppk, vd_ppk   # 默认
        ml_prior_used = False
        try:
            # 需要 predict_cl_vd_from_ml 函数已在工具函数中定义
        ...
        target_desc = f"{target_conc} mg/L（{'手动' if use_direct_conc else f'{4 if severity_str=="严重" else 1}×MIC'}）"
