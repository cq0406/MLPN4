...
if st.session_state.show_stats:
    ...
    if df_raw.empty:
        ...
        if analysis_mode == "📊 Routine Indicator Analysis":
            ...
            st.subheader("📈 Covariate Analysis for Target Attainment")
            if df_raw.empty:
                st.warning("No data available")
            else:
                # 中→英特征名映射（确保图表不出现中文）
                ...
                if not num_cols:
                    st.warning("No numeric covariates found. Please import data first.")
                else:
                    ...
                    ax.set_xlabel("Group")
                    ax.set_ylabel(selected_cov_en)          # 英文坐标轴标签
                    ax.set_title(f"{selected_cov_en} by Group")   # 英文标题
                    ax.spines['top'].set_visible(False)
                    ax.spines['right'].set_visible(False)
                    st.pyplot(fig)
...
if st.button("✅ 生成个体化给药方案"):
    ...
    try:
        ...
        # 确保 severity 已定义（即使在提前退出前也会被定义，但此处双重保险）
        severity = 感染程度 if 感染程度 not in ["请选择", ""] else "严重"

        if user_target_conc > 0:
            # 手动输入了目标浓度，直接以谷浓度达标为准
            target_conc = user_target_conc
            target_mic = raw_mic
            use_direct_conc = True
        else:
